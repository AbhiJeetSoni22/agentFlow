...
import {
  ...
  successToast,
  warningToast,
} from "@/components/Utils/ShambhoToast";
...
const NodeEditorSidebar = ({
  ...
  useEffect(() => {
    const isFunctionalOrIssueAgent =
      localNodeData.userAgentName === "functionalAgent" ||
      localNodeData.userAgentName === "issueIdentifier";
    if (isFunctionalOrIssueAgent && !hasAddedIssueIdentifier) {
      const issueNode = nodes.find(
        (node) =>
          node.data?.agent?.agentName?.toLowerCase() === "issueidentifier"
      );
      if (issueNode) {
        setFunctions((prevFunctions) => {
          const exists = prevFunctions.some(
            (func) => func._id === "issueIdentifier"
          );
          if (!exists) {
    ...
    if (aiAgentData.agentId) {
      fetchFunctionsByBotId(aiAgentData.agentId).then(() => {
        const isFunctionalOrIssueAgent =
          ...
          localNodeData.agentName === "issueIdentifier";
        if (isFunctionalOrIssueAgent) {
          ...
          if (
            issueNode &&
            !functions.some((f) => f._id === "issueIdentifier")
          ) {
            setFunctions((prev) => [
              ...prev,
              {
  ...
  const getFilteredFunctions = () => {
    ...
    if (localNodeData.agentName === "functionalAgent") {
      ...
      filteredFunction = functions.filter(
        (func) => func._id !== "issueIdentifier"
      );
    }
  ...
  const handleRemoveFunction = (func) => {
    setLocalNodeData((prev) => ({
      ...prev,
      availableFunctions: prev.availableFunctions.filter(
        (f) => f.id !== func.id
      ),
    }));
  ...
  const getAgentDisplayName = (executeUserAgentId, nodes) => {
    ...
    const foundNode = nodes.find(
      (node) =>
        node.data.agent.userAgentName === executeUserAgentId ||
        node.id === executeUserAgentId
    );

    return (
      foundNode?.data?.agent?.displayAgentName ||
      foundNode?.data?.agent?.agentName ||
      "Not found"
    );
  };
      ...
      >
        <div className=" p-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold truncate">
            Edit Node: {localNodeData.displayAgentName}
          </h2>
          <button
            onClick={onCancel}
        ...
        <div className="px-4 overflow-y-auto flex-1 h-full scrollbar-thin space-y-4">
          ...
          >
            ...
            {/* Agent Prompt */}
            {nodeData.agentName === "functionalAgent" && (
              <div>
                <label className="block text-sm font-semibold mb-2">
                  Agent Prompt
                </label>
          ...
          {["functionalAgent", "issueIdentifier", "reactAgent"].some((prefix) =>
            localNodeData.agentName?.startsWith(prefix)
          ) && (
            <div
              style={{
                backgroundColor: colors.backgroundSecondary,
                  ...
                  <>
                    ...
                    {localNodeData.availableFunctions.length > 0 && (
                      <div className="flex flex-wrap items-center justify-start mb-4 max-h-40 gap-2 overflow-y-auto">
                        {localNodeData.availableFunctions.map((func, index) => (
                          <div
                            key={func.id || index}
                            className="w-fit flex justify-between items-center rounded-lg px-3 py-2 border"
                            style={{
                              backgroundColor: colors.backgroundPrimary,
                    ...
                    <div className="mt-4">
                      ...
                      >
                        ...
                        {getFilteredAgentDisplayNames(nodes, localNodeData).map(
                          (displayName, index) => (
                  ...
                  <div key={index} className="mb-2">
                    ...
                    >
                      <span className="text-sm">
                        {nodeData?.displayAgentName ||
                          nodeData?.agentName ||
                          "No Name"}{" "}
                        →{" "}
                        {getAgentDisplayName(
                          condition.executeUserAgent,
                          nodes
                        ) || "None"}
                      </span>
