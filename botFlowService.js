...
export const sendToBotFlow = async ({ flow, agentId, token, flowDetails, type = 'NODE_RED', reactAgent }) => {
    ...
    try {
        ...
        const response = await axios.post(`${route}/?agentId=${agentId}&type=${type}`, payload,
            {
                headers: {
                    'auth-token': token
                }
...
export const updateBotFlow = async ({ flow, agentId, flowId, token, type = 'NODE_RED', reactAgent, flowDetails }) => {
    try {
...
export const toggleFlowStateService = async ({ flowId, isActive, token }) => {
    try {
        const response = await axios.put(`${route}/toggleFlowState/${flowId}`,
...
export const triggerNodeRedFlow = async ({ flowId, agentId, token }) => {
    try {
        const response = await axios.post('/api/nodeRed', {},
            {
                params: {
                    agentId: agentId,
                    flowId: flowId,
