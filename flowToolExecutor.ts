...
// Utility function to log LLM-related steps with metadata
async function logLLMStep(
  sessionId: string,
  type: string,
  content: string,
  prompt: string,
...
export async function executeReplyAgentNode(
  node: FlowNode
): Promise<string | undefined> {
  // Ye check karega ki node mein 'reply' naam ki field hai ya nahi (mai maan raha hoon ki FlowNode mein ek optional 'reply' field hoga)
  const replyMessage = node?.reply;

  // Yahan hum log bhi kar sakte hain (optional)
  console.log(`[Reply Agent] Returning hardcoded reply: ${replyMessage}`);

  // Agar reply message milta hai to use return karein
  if (replyMessage && typeof replyMessage === "string") {
    return replyMessage;
  }

  // Agar reply field nahi mila to undefined return karein, ya koi default error message
  return undefined;
}
export async function executeReactAgentNode(
  ...
  const updatedExecutingFlow = await AgentFlowState.findById(executingFlowId, {
    messages: 1,
    _id: -1,
    companyId: 1,
    nodes: 1,
  });
  const lastUserMessage = updatedExecutingFlow?.messages
    ?.slice()
    .reverse()
    .find((msg) => msg.owner === "User");
  const finalQuery = lastUserMessage?.message || initialQuery;
  const nodeExists = updatedExecutingFlow?.nodes?.some(
    (n) => n.userAgentName === node.userAgentName
  );
...
export class ToolExecutor {
  ...
  public static async assignLLMProperty(flowId: string) {
    ...
    const flow = await AgentFlow.findOne(
      { _id: flowId },
      { llmModel: 1, llmService: 1, _id: -1 }
    );
    if (flow) {
  ...
  private async updateFlowDocumentAndState(
    ...
    const nodeExists = existingFlow.nodes?.some(
      (n) => n.userAgentName === this.node?.userAgentName
    );
    ...
    if (!variableExists) {
      const functionParameters =
        this.availableTool.toolConfig.dynamicParams?.map((p: any) => ({
          variableName: p.key,
  ...
  private async functionalAgent(
    ...
    try {
      ...
      const latestVariableEntry = agentFlow?.variables?.find(
        (v) => v.userAgentName === this.node?.userAgentName
      );
      ...
      const { parametersJsonString, prompt, response } =
        await this.getLLMUpdatedParameters(query, latestVariableEntry); // <--- Destructure prompt and response
      const updatedFunctionParameters = JSON.parse(parametersJsonString);

      // Log the parameter gathering LLM call
      await logLLMStep(
        this.sessionId,
        "PARAMETERS_GATHERING_LLM_CALL",
        `LLM called to extract parameters for tool: ${this.availableTool.toolName}.`,
        prompt,
        response
      ...
      const missingParams = updatedFunctionParameters.filter(
        (param: any) =>
          param.variableValue === null || param.variableValue === ""
      );
  ...
  private async getLLMUpdatedParameters(
    query: string,
    latestVariableEntry: any
  ): Promise<{
    ...
    const llmResponse = await llmService.call(
      ...
      this.history,
      query,
      ToolExecutor.llmModel
    ); // <--- Get LLM response

    return { parametersJsonString: llmResponse, prompt, response: llmResponse }; // <--- Return prompt and response
  }
 ...
 6. Do NOT include any extra text, code blocks, or explanations outside the JSON array.`;
  ...
  private async updateParametersInDb(
    ...
    userReply?: string // <--- Added userReply
  ): Promise<void> {
    ...
    if (allParametersFilled && sessionId) {
      // Prepare metadata with userReply
      const metadata: any = {};
      if (userReply) {
        metadata.userReply = userReply; // <--- Save userReply in metadata
      }
        ...
        { sessionId: sessionId },
        {
          $push: {
            steps: {
              ...
              content: `All parameters for tool ${this.availableTool?.toolName} are gathered.`,
              timestamp: new Date(),
  ...
  private async handleToolExecutionAndConditions(
    ...
    const executionResult = await executeToolById(
      toolId,
      args,
      this.executingFlowObject!.userId
    );
    ...
    await Log.findOneAndUpdate(
      { sessionId: sessionId },
      {
        $push: {
          steps: {
            type: "NODE_EXECUTED",
            content: `Node execution completed for ${this.node?.userAgentName} with result: ${executionResult}.`,
            timestamp: new Date(),
          },
        },
      }
    ...
    await this.saveMessage(
      `The final result of the operation is: ${executionResult}`
    );
  ...
  private async getNextNodeIdFromLLM(
    ...
    const llmService = new LLMService(ToolExecutor.llmService);
    const prompt = `The result of node execution is ${result}. Here are the available conditions: ${JSON.stringify(
      conditions
    )}. Based on the result, which condition is met? Return only the 'executeAgent' value of the satisfied condition. For example, if the result is 30, and a condition says "result should be smaller than 50", you should return the corresponding 'executeAgent' ID. Return only the ID, nothing else.`;
    const llmResponse = await llmService.call(
      prompt,
      this.history,
      `Result: ${result}`,
