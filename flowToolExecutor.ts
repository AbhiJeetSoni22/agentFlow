...
export async function executeReplyAgentNode(node: FlowNode): Promise<string | undefined> {
 ...
 export class ToolExecutor {
    ...
    private async functionalAgent(
        ...
        try {
            ...
            const { parametersJsonString, prompt, response } = await this.getLLMUpdatedParameters(query, latestVariableEntry); // <--- Destructure prompt and response
    ...
    private async getLLMUpdatedParameters(query: string, latestVariableEntry: any): Promise<{ parametersJsonString: string, prompt: string, response: string }> { // <--- Updated return type
        
        
        const llmService = new LLMService(ToolExecutor.llmService);
        const prompt = this.createLLMPrompt(latestVariableEntry);
        this.history.push({ role: "user", content: query });
        const llmResponse = await llmService.call(prompt, this.history, query,ToolExecutor.llmModel); // <--- Get LLM response
