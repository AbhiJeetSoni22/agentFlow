...
export class ReActAgent {
  ...
  private async _loadLatestBotFlowAndTools() {
    console.log(
      `[ReAct Flow Loader] Fetching latest published REACT BotFlow for companyId: ${this.companyId}`
    );

    let botFlowQuery: any = {
    ...
    if (this.hardcodedFlowId) {
      console.log(
        `[ReAct Flow Loader] Overriding query with hardcoded flow ID: ${this.hardcodedFlowId}`
      );
    ...
    if (!botFlow || !botFlow.reactAgent) {
      throw new Error(
        `[ReAct Agent Run] No published REACT BotFlow found for companyId: ${this.companyId}`
      );
    }
    ...
    if (toolIds.length === 0) {
      console.warn(
        `[ReAct Agent Run] No tools specified in the latest BotFlow for companyId: ${this.companyId}`
      );
      this.dynamicTools = [];
      return;
    }
    ...
    if (validTools.length !== toolIds.length) {
      console.warn(
        `[ReAct Agent Run] Some tool IDs from BotFlow were not found in the Tools collection and have been skipped.`
      );
    }
  ...
  private createPrompt(query: string, history: string): string {
    if (
      !this.botFlow ||
      !this.botFlow.reactAgent ||
      !this.botFlow.reactAgent.systemPrompt
    ) {
  ...
  public async run(
    ...
    try {
      ...
      console.error(
        "[ReAct PRE-RUN] Failed to load BotFlow and Tools:",
        error.message
      );
  ...
  private async executeActionPhase(
    ...
    const toolActionsToExecute = pendingExecutions.map((t) => ({
      toolId: t.toolId,
    }));
