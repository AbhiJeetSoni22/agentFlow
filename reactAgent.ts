...
export class ReActAgent {
    ...
    public static async create(companyId: string,hardcodedFlowId?: string,agentId?:any): Promise<ReActAgent> {
        ...
        while (state.status === 'running' && iterations < maxIterations) {
            ...
            try {
                switch (state.currentPhase) {
                    case 'THOUGHT':
                        await this.executeThoughtPhase(state, query, socket, sessionId, agentId);
                        break;
                    case 'ACTION':
    ...
    private async executeThoughtPhase(state: IReactAgentState, query: string, socket: Socket, sessionId: string, agentId: string) {
        ...
        const history = state.steps.map((s:any) => `${s.type.toUpperCase()}: ${s.content}`).join("\n");
    ...
    private async executeActionPhase(endUserId: string, state: IReactAgentState, toolExecutor: ToolExecutor, socket: Socket, askUser: (q: string) => Promise<string>, initialQuery: string, sessionId: string, agentId: string) {
        ...
        const toolActionsToExecute = pendingExecutions.map((t:any) => ({ toolId: t.toolId }));
