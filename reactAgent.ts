...
async function addLogStep(
  sessionId: string,
  stepData: Omit<any, "stepNumber" | "_id" | "timestamp">
) {
...
export class ReActAgent {
  ...
  private async _loadLatestAgentFlowAndTools(agentId: any) {
    console.log(
      `[ReAct Flow Loader] Fetching latest published REACT AgentFlow for agentId: ${agentId}`
    );

    // Step 1: React Agent ka latest published AgentFlow fetch karo
    const agentFlow = await AgentFlow.findOne({
      agentId: agentId,
      flowType: "REACT",
      flowState: "PUBLISH",
    });
    if (!agentFlow || !agentFlow.reactAgent) {
      throw new Error(
        `[ReAct Agent Run] No published REACT AgentFlow found for agentId: ${agentId}`
      );
    }
    ...
    if (toolIds.length === 0) {
      console.warn(
        `[ReAct Agent Run] No tools specified in the latest AgentFlow for agentId: ${agentId}`
      );
      this.dynamicTools = [];
      return;
    }
    ...
    if (validTools.length !== toolIds.length) {
      console.warn(
        `[ReAct Agent Run] Some tool IDs from AgentFlow were not found in the Tools collection and have been skipped.`
      );
    }
    ...
    console.log(
      `[ReAct Flow Loader] Successfully loaded ${this.dynamicTools.length} tools.`
    );
  }

  private async _getResolvedToolForStateInit(
    toolId: string,
    companyId: string,
    agentId: string
  ): Promise<(ITool | IStandardTool) | null> {
    if (!toolId || !toolId.match(/^[0-9a-fA-F]{24}$/)) {
      console.error(
        `[ReActAgent State Init] Invalid toolId format: ${toolId}. Skipping resolution.`
      );
      return null;
    }
    ...
    if (isMcpSubTool) {
      console.log(
        `[ReActAgent State Init] Tool '${toolId}' is an MCP sub-tool. Fetching from Standard Tools.`
      );
      return await StandardToolModel.findById(toolId);
    }

    console.log(
      `[ReActAgent State Init] Tool '${toolId}' is a standard API/CRM tool. Fetching from Tools.`
    );
    return await ToolModel.findById(toolId);
  }

  private async getFriendlyObservation(
  ...
  private createPrompt(query: string, history: string): string {
    if (
      !this.agentFlow ||
      !this.agentFlow.reactAgent ||
      !this.agentFlow.reactAgent.systemPrompt
    ) {
      throw new Error("AgentFlow or system prompt is not loaded correctly.");
    }
    const reactAgentConfig = this.agentFlow.reactAgent;

    const toolsArray = this.dynamicTools
      .reduce((descriptions, tool) => {
        if (
          tool.toolType === "MCP" &&
          tool.toolConfig &&
          ...
          const mcpSubTools = tool.toolConfig.tools.map(
            (subTool: any) =>
              `- {"toolId": "${subTool.toolId}", "toolName": "${subTool.toolName}", "description": "${subTool.toolDescription}"}`
          );
          return descriptions.concat(mcpSubTools);
        } else {
          descriptions.push(
            `- {"toolId": "${tool._id}", "toolName": "${tool.toolName}", "description": "${tool.toolDescription}"}`
          );
          return descriptions;
        }
      }, [] as string[])
    ...
    systemPrompt = systemPrompt.replace(
      /\$\{history \|\| "No history yet\."\}/g,
      history || "No history yet."
    );
    systemPrompt = systemPrompt.replace(/\$\{query\}/g, query);

    const userPrompt =
      reactAgentConfig.userPrompt ||
      "Now, generate your response for the current user query.";
  ...
  private parseAIResponse(response: string): ParsedResponse | null {
    try {
      ...
      if (!jsonMatch) {
        console.error(
          "AI Response was not a JSON object. Response received:",
          response
        );
      ...
      return {
        ...
        toolActions: Array.isArray(parsed.toolActions)
          ? parsed.toolActions.map((a: any) => ({
              toolId: a.toolId || a.toolName || a.tool,
            }))
          : [],
      };
    } catch (e: any) {
  ...
  public async run(
    ...
    // Har run se pehle latest AgentFlow aur tools fetch karo
    try {
      await this._loadLatestAgentFlowAndTools(agentId);
    } catch (error: any) {
      console.error(
        "[ReAct PRE-RUN] Failed to load AgentFlow and Tools:",
        error.message
      );
      socket.emit("receiveMessageToUser", {
        message:
          "Sorry, I couldn't set up my tools correctly. Please contact support. 🛠️",
        sender: senderId,
        receiver: receiverId,
      });
    ...
    let findAgentState = await ReactAgentAgentState.findOne({
      companyId,
      agentId,
      endUserId,
      status: "running",
    ...
    while (state.status === "running" && iterations < maxIterations) {
      ...
      try {
        switch (state.currentPhase) {
          case "THOUGHT":
            await this.executeThoughtPhase(
              state,
              query,
              socket,
              sessionId,
  ...
  private async executeThoughtPhase(
    state: IReactAgentState,
    query: string,
    socket: Socket,
    sessionId: string,
    agentId: string
    ...
    const { llmService: llmServiceName, llmModel: llmModelName } =
      this.agentFlow.reactAgent;
    const llmService = new LLMService(llmServiceName);
    const aiResponse = await llmService.callGrok(
      prompt,
      [],
      query,
      llmModelName
    ...
    if (toolActions.length === 0 || !toolActions[0]?.toolId) {
      console.log(
        "[ReAct] Thought resulted in no valid action. Completing task."
      );
      state.status = "completed";
      ...
      for (const action of toolActions) {
        const toolDetails = await this._getResolvedToolForStateInit(
          action.toolId,
          state.companyId,
          agentId
        );
        if (toolDetails) {
          actionDetails.push(
            `{ toolId: ${action.toolId}, toolName: "${
              (toolDetails as any).toolName
            }", description: "${(toolDetails as any).toolDescription}" }`
          );
        } else {
          actionDetails.push(
            `{ toolId: ${action.toolId} (details not found) }`
          );
        }
      }
      ...
      for (const action of toolActions) {
        ...
        if (toolDetails) {
          const paramsArray = Array.isArray(
            (toolDetails as any)?.toolConfig?.dynamicParams
          )
            ? (toolDetails as any).toolConfig.dynamicParams
            : [];
          ...
          console.error(
            `[ReAct] Failed to resolve tool details for toolId: ${action.toolId}`
          );
        }
      }
  ...
  private async executeActionPhase(
    ...
    const pendingExecutions = state.toolExecutions.filter(
      (te: any) => te.status === "pending"
    );
    ...
    const lastThought = state.steps
      .filter((s: any) => s.type === "thought")
      .pop();

    if (lastThought && lastThought.content) {
      console.log(
        `[ReAct DEBUG] Thought mila hai user ko bhejne ke liye: "${lastThought.content}"`
      );
      socket.emit("receiveMessageToUser", {
      ...
      console.log(
        "[ReAct DEBUG] User ko bhejne ke liye koi thought nahi mila."
      );
    }

    const toolActionsToExecute = pendingExecutions.map((t: any) => ({
      toolId: t.toolId,
    }));
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      const technicalObservation = result.success
        ? result.observation ?? "Success"
        : `Error: ${result.error ?? "Unknown error"}`;
      const friendlyObservation = await this.getFriendlyObservation(
      ...
      const toolName = toolDetails
        ? (toolDetails as any).toolName
        : "Unknown Tool";
      const toolDescription = toolDetails
        ? (toolDetails as any).toolDescription
