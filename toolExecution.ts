...
export async function executeApiTool(tool: Tool, args: any, endUserId: string): Promise<number | undefined | string> {
    if (!tool || !args) {
...
export async function executeCrmTool(tool: Tool, args: any, endUserId: string): Promise<number | undefined | string> {
    if (!tool || !args) {
        console.error("❌ Error: Tool or arguments are missing.");
        return undefined;
    }
...
export async function executeToolById(toolId: string, args: any, endUserId: string): Promise<number | undefined | string> {
    try {
