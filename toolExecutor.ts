...
export async function executeReactAgentNode(
  ...
  // 🆕 Naya Code: Node ko AgentFlowState mein push karna
  // Pehle check karein ki node pehle se exist karta hai ya nahi
  const nodeExists = updatedExecutingFlow?.nodes?.some(
    (n) => n.userAgentName === node.userAgentName
  );

  if (!nodeExists) {
    // Agar node exist nahi karta, to naya node object banayein
    const currentNode: INode = {
      agentName: node.agentName,
      userAgentName: node.userAgentName,
      condition: node.condition,
      availableFunctions: node.availableFunctions,
      nodeState: "Running", // Node ka state 'Running' set karein
    };

    // Document ko $push operator ka use karke update karein
    ...
    console.log(`✅ ${node.userAgentName} node added to AgentFlowState.`);
  }
  ...
  const hardCoadedFlowId = node?.userAgentName;
  ...
  if (result === "complete" && node?.condition.length > 0) {
    if (node?.condition.length === 1) {
      console.log(
        "returning nodeuseragent from toolExecutor",
        node?.condition[0].executeUserAgent
      );
      return node?.condition[0].executeUserAgent;
    }

    console.log("node condition in reactAgent", node?.condition);
  }
