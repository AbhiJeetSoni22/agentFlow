...
      6. Do NOT include any extra text, code blocks, or explanations outside the JSON array.`;
      ...
      const stateUpdate = missingParams.length === 0 ? true : false; // update the specific variable entry in the database
  ...
  public async updateExecutingFlowDocument(
    ...
    const variableExists = existingFlow?.variables?.some(
      (v) => v.userAgentName === this.node?.userAgentName && v.state === false
    );
    if (!variableExists) {
